...
# Extract only the last part (folder name) of the model_id
model_name = model_id.split('/')[-1]  
output_file_path = os.path.join(output_dir, f"eds_{dataset_name}_{model_name}.jsonl")
...
with open(output_file_path, "w", encoding='utf-8') as output_file:
    for entry in filtered_entries:
        ...
        outputs = pipeline(
            ...
            num_beams=45,  # Use beam search with 40 beams
            num_return_sequences=20,  # Ensure 20 sequences are returned
            early_stopping=True,
            repetition_penalty=1.2,  # Penalize repetition (adjust value as needed)
        )
