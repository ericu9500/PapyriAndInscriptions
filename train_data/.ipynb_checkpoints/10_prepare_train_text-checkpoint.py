...
for repeat_index in range(4):  # Repeat the process 4 times for different outputs
    output_entries = []

    for entry in entries:
        for _ in range(5):  # Repeat 5 times with brackets and 5 times without
            for edition_field in ["Edition_with_brackets", "Edition_without_brackets"]:
                if edition_field in entry:
                    ...
                    # Create the JSON object for output
                    output_entry = {
                        "messages": [
    ...
    # Write each iteration's output to a different JSONL file
    output_filename = os.path.join(output_dir, f'DDbDPtrain_eds_{repeat_index + 1}.jsonl')
