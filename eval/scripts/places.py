...
parser.add_argument("--dataset", type=str, required=True, help="Path to the dataset file (JSONL) or Huggingface dataset identifier.")
...
parser.add_argument("--num_beams", type=int, default=20, help="Number of beams for beam search (default: 1).")
parser.add_argument("--from_huggingface", action="store_true", help="Whether to load dataset from Hugging Face Hub.")
...
allowed_prompts = [
    "Assign this papyrus fragment to a place!",
    "Assign this inscription to a place!",
    "Assign this papyrus fragment to an exact place!",
...
dataset_name = os.path.basename(args.dataset).split('.')[0] if not args.from_huggingface else args.dataset.replace("/", "_")
model_name = args.model.split('/')[-1]
output_file_path = os.path.join(args.output_folder, f"places_{dataset_name}_{model_name}.jsonl")
